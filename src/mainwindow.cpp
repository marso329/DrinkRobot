...
database	= new DataBase();
	...
	ui->scoreboard_heighest_list->horizontalHeader()->setSectionResizeMode(
			QHeaderView::Stretch);
	ui->scoreboard_heighest_list->setItemDelegate(new NotEditableDelegate());
	...
	ui->scoreboard_drinkmost_list->horizontalHeader()->setSectionResizeMode(
			QHeaderView::Stretch);
	ui->scoreboard_drinkmost_list->setItemDelegate(new NotEditableDelegate());
	...
	ui->scoreboard_spentmost_list->horizontalHeader()->setSectionResizeMode(
			QHeaderView::Stretch);
	ui->scoreboard_spentmost_list->setItemDelegate(new NotEditableDelegate());
	...
	user_model->setHorizontalHeaderItem(2,
			new QStandardItem(QString("Weight[kg]")));
	...
	ui->adduser_list->setItemDelegateForColumn(0, new NotEditableDelegate());
	...
	connect(ui->adddrink_adddrink, SIGNAL(clicked()), this, SLOT(add_drink()));

	//connect(ui->adddrink_drinkselect, SIGNAL(currentTextChanged(const QString &)), this,
	//	SLOT(drink_changed(const Qstring&)));

	connect(ui->adddrink_drinkselect, &QComboBox::currentTextChanged, this,
			&MainWindow::drink_changed);

	connect(ui->adddrink_addingredient, &QPushButton::clicked, this,
			&MainWindow::add_ingredient_to_drink);

	connect(drink_model, &QStandardItemModel::dataChanged, this,
			&MainWindow::drinkDataChanged);

	connect(ui->adddrink_select_icon, &QPushButton::clicked, this,
			&MainWindow::select_icon);
	...
	connect(ui->settemp_tank0_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank1_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank2_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank3_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank4_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank5_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank6_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank7_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank8_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));
	connect(ui->settemp_tank9_set, SIGNAL(valueChanged(int)), this,
			SLOT(set_temp_changed(int)));

	connect(ui->scoreboard, SIGNAL(clicked()), this, SLOT(setup_scoreboard()));
...
void MainWindow::drink_selected() {
	...
	promilletext << "Hello " << current_user
			<< " your promille level before this drink was \n" << promille
			<< " and after this drink it will be " << new_promille;
	loading->promille_label->setText(
			QString::fromStdString(promilletext.str()));
...
void MainWindow::drinkDataChanged(const QModelIndex &topLeft,
	...
	for (int i = 0; i < drink_model->rowCount(); i++) {
		...
		std::string ingredient =
				drink_model->data(index).toString().toStdString();
...
void MainWindow::add_drink() {
	...
	set_add_drink();
	int index = ui->adddrink_drinkselect->findText(
...
void MainWindow::drink_changed(const QString &text) {
	...
	drink_model->setHorizontalHeaderItem(1,
			new QStandardItem(QString("Amount [cl]")));
	ui->adddrink_list->setItemDelegateForColumn(0,
			new ComboBoxItemDelegate(allingredients));
	int ind = 0;
	for (auto it = ingredients.begin(); it != ingredients.end(); it++) {
		...
		temp << amount;
		//add amount
		drink_model->setItem(ind, 1,
				new QStandardItem(QString::fromStdString(temp.str())));
...
void MainWindow::setup_scoreboard() {
	scoreboard_heighest_model->removeRows(0,
			scoreboard_heighest_model->rowCount());
...
void MainWindow::set_add_user() {
	...
	for (auto it = users.begin(); it != users.end(); it++) {
		...
		//newly added
		QString weight_str = QString::fromStdString(
				std::to_string(database->getWeight(name_std)));
